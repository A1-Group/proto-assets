...
class TopicHandler:
    ...
    def handle_get_playlist(self, upload_url: str):
        ...
        try:
            ...
            playlist_file_name = urllib.parse.unquote(parsed_path.split('/')[-1])

            self.logger.debug(f"Extracted S3 filename from URL: {playlist_file_name}")

            # files = {'file': (playlist_file_name, response.content)}
            self.logger.debug(f"Uploading playlist to: {upload_url}")

            # Raw body instead of multipart because a pre-signed PUT stores the body verbatim
            # upload_response = requests.put(upload_url, files=files, timeout=60)
